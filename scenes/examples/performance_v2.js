...
async function render(context) {
  ...
  if (context.frametime !== undefined && context.frametime > 0) {
    frameTimes.push(context.frametime);
  ...
  if (mode === 'burst') {
    ...
    const fps = avgFrametime > 0 ? Math.round(1000 / avgFrametime) : 0;
    const currentFrametime = context.frametime || 0;
  ...
  if (shouldRender) {
    ...
    if (now - lastChartUpdate >= 100 && chartX < 64) {
      ...
      const baseColor = getSimplePerformanceColor(frametime);
  ...
  const frametimeDelay = context.frametime || testInterval;
  ...
  await device.push('test_performance_v2', context.publishOk);
}
