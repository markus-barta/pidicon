...
module.exports = {
  ...
  render: async (ctx) => {
    ...
    const isFreshStart =
      !startTime || (isNewMessage && !getState('testCompleted'));
    ...
    if (isFreshStart) {
      ...
      if (!deviceReady) {
        ...
        if (stillNotReady) {
          console.warn(
            `⚠️ [TEST V2] Device still not ready, proceeding anyway`,
          );
        }
      }
    ...
    const loopEndTime =
      mode === 'loop' ? startTime + loopDuration : startTime + 60000; // Cap at 60 seconds
    ...
    let shouldRender =
      mode === 'sweep' ? !getState('testCompleted') : now <= loopEndTime; // Keep rendering until loop duration expires
    ...
    if (mode === 'burst') {
      ...
      const remainingIterations = Math.max(
        0,
        63 - (chartX - CHART_CONFIG.CHART_START_X),
      );
      ...
      const progressPercent = Math.min(
        100,
        Math.round((pointsCollected / 64) * 100),
      );
    ...
    if (shouldRender) {
      ...
      for (
        ...
        await device.drawPixelRgba(
          [0, y],
          [...CHART_CONFIG.AXIS_COLOR.slice(0, 3), CHART_OPACITY],
        );
      }
    ...
    if (ctx.frametime !== undefined && frameTimes.length % 10 === 0) {
      console.log(
        `🎯 [PERF TEST V2] ${mode} mode, interval:${currentInterval}ms, frametime:${ctx.frametime}ms, avg:${Math.round(avgFrametime)}ms, samples:${frameTimes.length}, shouldRender:${shouldRender}, elapsed:${Math.round(elapsed / 1000)}s`,
      );
    }
    ...
    const shouldContinue =
      (useAdaptiveTiming && shouldRender && !state.stop) ||
    ...
    if (shouldContinue) {
      ...
      console.log(
        `🔄 [LOOP V2] Chart point ${chartPoints}/63, frametime: ${ctx.frametime}ms, delay: ${nextMessageDelay}ms (${delayType}), remaining: ${remainingPoints} points, ~${estimatedSeconds}s`,
      );
      ...
      const loopTimer = setTimeout(() => {
        try {
          ...
          console.log(
            `📊 [${mode.toUpperCase()} V2] Stack trace: ${err.stack}`,
          );
          setState('loopScheduled', false); // Reset flag on error
        }
      }, nextMessageDelay);
...
async function drawTextRgbaAlignedWithBg(
  ...
  if (clearBg) {
    ...
    await device.drawRectangleRgba(
      [bgX, y],
      [Math.min(textWidth, 64 - bgX), textHeight],
      [0, 0, 0, 255],
    );
  }
