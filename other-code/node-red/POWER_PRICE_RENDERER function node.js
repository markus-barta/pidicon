  ...
  initialize(msg) {
    ...
    if (!this.targetDeviceHost) {
      node.status({
        fill: 'red',
        shape: 'ring',
        text: 'Missing target hostAddress',
      });
      throw new Error(
        'No target device host address provided in msg.payload.hostAddress',
      );
    }
    ...
    if (!this.device) {
      node.status({
        fill: 'red',
        shape: 'ring',
        text: `Device ${this.targetDeviceHost} not found`,
      });
      node.warn(
        `Pixoo device '${this.targetDeviceHost}' not found in PixooManager. Was it initialized?`,
      );
      throw new Error(
        `Pixoo device '${this.targetDeviceHost}' not found or not initialized.`,
      );
    }
    ...
    if (!this.api) {
      node.status({
        fill: 'red',
        shape: 'ring',
        text: `Device ${this.targetDeviceHost} API missing`,
      });
      node.warn(
        `Pixoo device '${this.targetDeviceHost}' found, but API instance is missing.`,
      );
      throw new Error(
        `Pixoo device '${this.targetDeviceHost}' API instance is missing.`,
      );
    }
...
const TimeUtils = {
  getCurrentTimeInfo() {
    ...
    const isDayTimeResult = context.global.isDaytime
      ? context.global.isDaytime()
      : { success: false };
    return {
      currentDate,
  ...
  getMoonPhaseFilename() {
    ...
    if (!suncalc?.getMoonIllumination) {
      node.warn(
        'SunCalc library or getMoonIllumination not available - using default moon image.',
      ); // Use node.warn directly if needed here
...
const Logger = {
  debug(message) {
    ...
    node.warn(
      `[Pixoo:${DeviceManager.targetDeviceHost || 'Unknown'}] ${message}`,
    );
  },
  ...
  setStatus(text, type = 'success') {
    ...
    try {
      ...
      const timeInfo = context.global.getLocalTime
        ? context.global.getLocalTime()
        : null;
...
const DisplayConfig = {
  ...
  CHART: {
    ...
    PV: {
      // PV Chart specific settings
      PREDICTION: {
        // Position shifts left by 24px in the afternoon
        position: TimeUtils.getCurrentTimeInfo().isAfterNoon
          ? [14 - 24, 10]
          : [14, 10],
      ...
      ACTUAL: {
        ...
        colors: {
          // Gradient colors for actual bars (currently solid yellow)
          start: [255, 255, 0, 255],
          end: [255, 255, 0, 255],
        },
        alpha: {
          // Alpha range for blending the top pixel of actual bars
          min: 25,
          max: 255,
        },
      },
    ...
    POWER_PRICE: {
      // Power Price Chart specific settings
      ...
      BLINK: {
        // Settings for blinking effect on certain price bars
      ...
      GRID: {
        ...
        spacing: {
          // Calculated spacing, cached here
          xDelta: null,
          yDelta: null,
        },
      },
      ...
      ZERO_LINE_MARKER: {
        // Configuration for the marker dot on the zero line <<< ADDED CONFIG SECTION
      ...
      settings: {
        // General settings for price chart drawing logic
  ...
  IMAGES: {
    ...
    BATTERY: {
      ...
      alpha: 250,
      fillframe: [1, 1, 10, 3],
    }, // fillframe: [x, y, w, h] relative to position
    CENT_SIGN: {
    ...
    SUN: {
      ...
      position: TimeUtils.getCurrentTimeInfo().isAfterNoon
        ? [32 - 24, 19]
        : [32, 19],
      alpha: TimeUtils.getCurrentTimeInfo().isDaytime ? 225 : 128,
    }, // Alpha changes day/night
    MOON: {
      path: TimeUtils.getMoonPhaseFilename(),
      dimensions: [5, 5],
      position: TimeUtils.getCurrentTimeInfo().isAfterNoon
        ? [58 - 24, 21]
        : [58, 21],
      alpha: TimeUtils.getCurrentTimeInfo().isDaytime ? 225 : 255,
    }, // Dynamic path, Alpha changes day/night
    CHARGE_LIGHTNING: {
  ...
  CLOCK: {
    ...
    separator: {
      // Settings for the blinking ':' separator
    ...
    shadow: {
      // Settings for clock text shadow
      enabled: true,
  ...
  TEXTS: {
    ...
    PV_TOTAL: {
      position: TimeUtils.getCurrentTimeInfo().isAfterNoon
        ...
        : [11, 13],
      color: [255, 255, 0, 100],
    }, // Actual PV total text (position shifts)
    PV_PREDICTION_TOTAL: {
      position: TimeUtils.getCurrentTimeInfo().isAfterNoon
        ? [50, 13]
        : [62, 13],
      color: [5, 5, 5, 255],
    }, // Predicted PV total text (position shifts)
    // Labels for the price chart Y-axis
    LABEL0: {
      position: [9, GRID_ZERO_LINE_Y - 2],
      color: [155, 155, 155, 250],
      text: '0',
    },
    LABEL10: {
      position: [9, GRID_ZERO_LINE_Y - 12],
      color: [155, 155, 155, 220],
      text: '10',
    },
    LABEL20: {
      position: [9, GRID_ZERO_LINE_Y - 22],
      color: [155, 155, 155, 190],
      text: '20',
    },
    LABEL_MORE: {
      position: [5, GRID_ZERO_LINE_Y - 32],
      color: [155, 155, 155, 160],
      text: '+',
    }, // Indicator for prices > 20
    // UVI text next to sun logo
    UVI: {
      position: TimeUtils.getCurrentTimeInfo().isAfterNoon
        ? [32 - 24 + 11, 19 + 2]
        : [32 + 11, 19 + 2], // x: sun.x + sun.width, y: sun.y + 1
      color: [
        148,
        0,
        211,
        TimeUtils.getCurrentTimeInfo().isDaytime ? 200 : 128,
      ],
    }, // Neon violet, alpha varies by daytime
  },
}; // End DisplayConfig
...
const DrawingUtils = {
   ...
   */
  async drawVerticalGradientLine(
    device,
    startPoint,
    endPoint,
    startColor,
    ...
    if (inputStartY <= inputEndY) {
      // Conceptual start is at or above conceptual end (usually drawing physically top-down)
      physicalTopY = inputStartY;
      physicalBottomY = inputEndY;
      colorAtPhysicalTop = inputStartColor;
      colorAtPhysicalBottom = inputEndColor;
    } else {
      // Conceptual start is below conceptual end (usually drawing physically bottom-up)
    ...
    const targetColorForAlphaBase =
      alphaTarget === 'end' ? inputEndColor : inputStartColor;
    ...
    for (let i = 0; i < length; i++) {
      ...
      try {
        ...
        if (useSpecialAlpha && currentY === targetY) {
          ...
          if (
            !Array.isArray(targetColorForAlphaBase) ||
            targetColorForAlphaBase.length < 3
          ) {
            Logger.debug(
              `Invalid targetColorForAlphaBase for alpha blend: ${JSON.stringify(targetColorForAlphaBase)}`,
            );
            continue; // Skip this pixel if base color is invalid
          }
          const finalAlpha = Math.max(0, Math.min(255, Math.round(pixelAlpha)));
          const specialColor = [
            ...targetColorForAlphaBase.slice(0, 3),
            finalAlpha,
          ];
        ...
        if (
          !Array.isArray(colorAtPhysicalTop) ||
          !Array.isArray(colorAtPhysicalBottom) ||
          colorAtPhysicalTop.length < 3 ||
          colorAtPhysicalBottom.length < 3
        ) {
          Logger.debug(
            `Invalid physical colors for gradient: Top=${JSON.stringify(colorAtPhysicalTop)}, Bottom=${JSON.stringify(colorAtPhysicalBottom)}`,
          );
        ...
        if (color.length === 3) {
          const defaultAlpha =
            Array.isArray(colorAtPhysicalTop) &&
            typeof colorAtPhysicalTop[3] === 'number'
              ? colorAtPhysicalTop[3]
              : 255;
          color.push(defaultAlpha);
        } else if (color.length > 3) {
          // Correct potential map issue
          color[3] =
            Array.isArray(colorAtPhysicalTop) &&
            typeof colorAtPhysicalTop[3] === 'number'
              ? colorAtPhysicalTop[3]
        ...
        Logger.debug(
          `Error drawing gradient pixel at [${x}, ${currentY}]: ${error.message}`,
        );
      }
    } // End for loop
  ...
  calculateTopPixelAlpha(remainder, minAlpha, maxAlpha) {
    ...
    const interpolatedAlpha =
      minAlpha + (maxAlpha - minAlpha) * cappedRemainder;
...
const DisplayComponents = {
  ...
  async renderBatteryFill(device, statusImgAlpha) {
    ...
    if (SONNEN_STATUS?.BatteryCharging) {
      await device.drawImageWithAlpha(
        DisplayConfig.IMAGES.CHARGE_LIGHTNING.path,
        DisplayConfig.IMAGES.CHARGE_LIGHTNING.position,
        DisplayConfig.IMAGES.CHARGE_LIGHTNING.dimensions,
        statusImgAlpha,
      ); // <-- Use parameter
    } else if (SONNEN_STATUS?.BatteryDischarging) {
      await device.drawImageWithAlpha(
        DisplayConfig.IMAGES.DISCHARGE_LIGHTNING.path,
        DisplayConfig.IMAGES.DISCHARGE_LIGHTNING.position,
        DisplayConfig.IMAGES.DISCHARGE_LIGHTNING.dimensions,
        statusImgAlpha,
      ); // <-- Use parameter
    }
  },
  ...
  async renderClock(device) {
    const timeData = context.global.getLocalTime
      ? context.global.getLocalTime().data?.strings
      : null;
    if (!timeData) {
      Logger.debug('Time data unavailable for clock.');
      return;
    }
    const {
      hour: hourPos,
    ...
    if (DisplayConfig.CLOCK.shadow.enabled) {
      const { offset: shadowOffset, color: shadowColor } =
        DisplayConfig.CLOCK.shadow;
      await device.drawTextRgbaAligned(
        hour,
        [hourPos[0], hourPos[1] + shadowOffset],
        shadowColor,
        'left',
      );
      await device.drawTextRgbaAligned(
        separator,
        [sepPos[0], sepPos[1] + shadowOffset],
        [...shadowColor.slice(0, 3), Math.min(alpha, shadowColor[3])],
        'left',
      );
      await device.drawTextRgbaAligned(
        minute,
        [minPos[0], minPos[1] + shadowOffset],
        shadowColor,
        'left',
      );
    }
  ...
  async renderPvActualBars(device) {
    ...
    if (!Array.isArray(dailyData) || !dailyData.length) {
      Logger.debug('PV actual data invalid.');
      return;
    }
    ...
    for (let hourIndex = 0; hourIndex < dailyData.length; hourIndex++) {
      ...
      const totalPixelsClamped = Math.min(
        chartHeight,
      ...
      if (remainder > 0) {
        partialPixelAlpha = DrawingUtils.calculateTopPixelAlpha(
          // Utility calculates alpha 0-255 from remainder 0-1
          remainder,
  ...
  async renderPvPredictionTotal(device) {
    ...
    const predictionWidth = await device.drawCustomFloatText(
      totalKwh,
      position,
      color,
      'right',
   ...
   */
  async renderPvTotal(device, slashXPosition) {
    // Accept slash position
    const dailyData = global.get('dailyPvDataActual') || [];
    if (!Array.isArray(dailyData)) {
      Logger.debug('PV actual data invalid.');
      return;
    ...
    const actualXPosition = [
      TimeUtils.getCurrentTimeInfo().isAfterNoon && slashXPosition !== null
        ? slashXPosition - offsetBeforeSlash
        : position[0],
      position[1],
    ];
    await device.drawCustomFloatText(
  ...
  async renderPowerPriceBars(device) {
    ...
    const timeInfo = context.global.getLocalTime
      ...
      : null;
    if (!timeInfo?.success) {
      Logger.debug('Local time data unavailable.');
      return;
    }
    ...
    const relevantPrices = this.filterAndSortPrices(
      priceInfo.data,
      referenceHourKey,
      currentHourKey,
      config.settings.maxHoursToDraw,
    );
  ...
  filterAndSortPrices(data, referenceKey, currentKey, maxHours) {
    ...
    if (DisplayConfig.DEBUG_LARGE_VALUES) {
      /* Adapt debugging logic if needed */
    }
    if (DisplayConfig.DEBUG_NEGATIVE_VALUES) {
      /* Adapt debugging logic if needed */
    }
  ...
  async drawPriceChart(device, prices, config) {
    ...
    for (let i = 0; i < prices.length; i++) {
      ...
      if (markerConfig.enabled) {
        ...
        await this._drawZeroLineMarker(
          device,
          x,
          zeroLineY,
          markerConfig.color,
          isPastHour,
          markerConfig.pastAlpha,
        ); // <<< CALL UPDATED
      }
      ...
      if (pricePoint.isCurrent && indicatorConfig.enabled) {
        ...
        if (
          yBase < DisplayConfig.DIMENSIONS.HEIGHT &&
          x > 0 &&
          x < DisplayConfig.DIMENSIONS.WIDTH - 1
        ) {
          try {
            ...
            Logger.debug(
              `Error drawing current hour indicator at x=${x}: ${indicatorError.message}`,
            );
          }
        } else {
          // Log if indicator would be off-screen (unlikely with current layout)
          Logger.debug(
            `Current hour indicator skipped at x=<span class="math-inline">\{x\}, y\=</span>{yBase} (out of bounds)`,
          );
        }
      }
   ...
   * @returns {Promise<void>}
   */
  async _drawZeroLineMarker(device, x, y, baseColor, isPastHour, pastAlpha) {
    // <<< SIGNATURE CHANGED
    try {
  ...
  async _renderPriceBar(device, priceData, config, x, blinkState) {
    ...
    const alphaValues = this._calculateBarAlphaValues(
      priceData,
      config,
      blinkState,
    );
  ...
  _calculateBarAlphaValues(priceData, config, blinkState) {
    ...
    let topPixelAlpha = this.calculatePriceBarTopPixelAlpha(
      priceData,
      config.colors,
    );
  ...
  calculateChartDimensions(config) {
    return {
      xStart: config.position[0],
      maxPixels: Math.min(
        DisplayConfig.DIMENSIONS.WIDTH - config.position[0],
  ...
  isValidPrice(priceData, lastKnownPrice) {
    return typeof priceData?.price === 'number';
  },
  ...
  determineBarColors(priceData, colorConfig) {
    ...
    if (price === null) {
      return { startColor: [50, 50, 50, 100], endColor: [50, 50, 50, 100] };
    } // Default grey for invalid
    if (isCurrent) {
      return {
   ...
   */
  calculatePriceBarTopPixelAlpha(priceData, colorConfig) {
    // <-- Renamed function
    ...
    let calculatedAlpha = DrawingUtils.calculateTopPixelAlpha(
      remainder,
      0,
      baseEndAlpha,
    ); // <-- Use utility
    ...
    if (
      priceData.isPastHour &&
      !priceData.isCurrent &&
      !priceData.isAmongCheapest
    ) {
  ...
  getBaseAlpha(priceData, colorConfig) {
    ...
    if (isAmongCheapest) {
      return isPastHour
        ? (colorConfig.cheapHours.past.end[3] ?? defaultAlpha)
        : (colorConfig.cheapHours.future.end[3] ?? defaultAlpha);
    }
    return isPastHour
      ? (colorConfig.pastHours.end[3] ?? defaultAlpha)
      : (colorConfig.futureHours.end[3] ?? defaultAlpha);
  },
  ...
  async drawPriceBar(device, barData) {
    try {
      ...
      Logger.debug(
        `Price bar rendering error at x=${barData?.x}: ${error.message}`,
      );
    }
  },
  ...
  _isZeroPrice(barData) {
    ...
    return (
      barData.fullPixels === 0 &&
      barData.topPixelAlpha === undefined &&
      !barData.isNegative
    );
  },
  ...
  _calculateColor(
    baseColor,
    isPastHour,
    pastAlphaPercent = DisplayConfig.NEGATIVE_PRICES.PAST_ALPHA_PERCENT,
  ) {
    // If not past hour, return the original color
    // Ensure baseColor is valid
    if (!Array.isArray(baseColor) || baseColor.length < 4) {
      Logger.debug(
        `Invalid baseColor in _calculateColor: ${JSON.stringify(baseColor)}`,
      );
  ...
  async _drawNegativePrice(device, barData) {
    ...
    const baseColor = DisplayConfig.CHART.POWER_PRICE.colors.negative || [
      0, 255, 0, 255,
    ];
    ...
    for (let i = 0; i < negativePixels.length; i++) {
      ...
      await device
        .drawPixelRgba([x, pixelY], color)
        .catch((err) =>
          Logger.debug(
            `Negative pixel error at [${x}, ${pixelY}]: ${err.message}`,
          ),
        );
    }
  },
  ...
  async _drawZeroPrice(device, barData) {
    ...
    const baseColorConfig = DisplayConfig.CHART.POWER_PRICE.colors.negative || [
      0, 255, 0, 255,
    ];
    ...
    if (isPastHour) {
      ...
      const pastAlphaPercent =
        DisplayConfig.NEGATIVE_PRICES.PAST_ALPHA_PERCENT || 50; // Get percentage
      finalAlpha = Math.max(
        0,
        Math.min(255, Math.round((pastAlphaPercent / 100) * 255)),
      ); // Calculate and clamp
    } else {
    ...
    await device
      .drawPixelRgba([x, zeroLineY], finalZeroDotColor)
      .catch((err) =>
        Logger.debug(
          `Zero price indicator error at [${x}, ${zeroLineY}]: ${err.message}`,
        ),
      );
  },
  ...
  async _drawPositiveBar(device, barData) {
    const { x, bottomY, fullPixels, topPixelAlpha, startColor, endColor } =
      barData;
    ...
    await DrawingUtils.drawVerticalGradientLine(
      ...
      Logger.debug(`Positive bar error at x=${x}: ${err.message}`),
    );
  },
  ...
  async _drawOverflowPixels(device, barData) {
    ...
    const baseColor = DisplayConfig.CHART.POWER_PRICE.colors.overflow || [
      255, 0, 0, 255,
    ];
    // Create final color using calculated overflowAlpha (handles blinking)
    // Ensure overflowAlpha is a number
    const finalOverflowAlpha =
      typeof overflowAlpha === 'number' ? overflowAlpha : baseColor[3];
    ...
    for (const pixel of overflowPixels) {
      ...
      await device
        ...
        .catch((err) =>
          Logger.debug(
            `Overflow pixel error at [${x}, ${pixelY}]: ${err.message}`,
          ),
        );
    }
  },
...
const DisplayController = {
  ...
  validateDeviceState({ currentMode, currentAction, allowedMode }) {
    ...
    if (!device)
      throw new Error(
        'DeviceManager not initialized before validateDeviceState call.',
      );
    if (
      currentAction === device.constructor.ACTIONS.PAUSE ||
  ...
  async renderGrid() {
    ...
    if (!config) {
      Logger.debug('Grid config missing.');
      return;
    } // Safety check
    ...
    if (!config.spacing.xDelta || !config.spacing.yDelta) {
      ...
      config.spacing.xDelta =
        vCount > 1
          ? Math.floor((config.endPos[0] - config.startPos[0]) / (vCount - 1))
          : 0;
      config.spacing.yDelta =
        hCount > 1
          ? Math.floor((config.endPos[1] - config.startPos[1]) / (hCount - 1))
          : 0;
    }
    ...
    if (config.horizontal?.count > 0 && config.horizontal?.color) {
      ...
      for (let i = 0; i < totalLines; i++) {
        ...
        const alpha = Math.min(
          255,
          minAlpha + Math.round((i / (totalLines - 1)) * alphaRange),
        );

        const color = [...config.horizontal.color.slice(0, 3), alpha];
        await device.drawLineRgba(
          [config.startPos[0] - config.overhang, y],
          [config.endPos[0] + config.overhang, y],
          color,
        );
      }
    }
    ...
    if (config.vertical?.count > 0 && config.vertical?.color) {
      for (let i = 0; i < config.vertical.count; i++) {
        ...
        await device.drawLineRgba(
          [x, config.startPos[1] - config.overhang],
          [x, config.endPos[1] + config.overhang],
          config.vertical.color,
        );
      }
    }
  },
  ...
  async renderBackground() {
    ...
    const imagesToDraw = [
      elements.PRICE_BACKGROUND,
      elements.PV_BACKGROUND,
      elements.BATTERY,
      elements.CENT_SIGN,
      elements.KWH_UNIT,
      elements.SUN,
    ...
    for (const image of imagesToDraw) {
      if (image && image.path) {
        try {
          ...
          Logger.debug(
            `Error drawing image ${image.path}: ${imgError.message}`,
          );
        }
      }
    }
  ...
  async render() {
    ...
    if (!device || !api)
      throw new Error('Device or API not available in render');
    ...
    let nextPriceAnimationIndex =
      (currentPriceAnimationIndex + 1) % (FLOW_ANI_INDEX_MAX + 1); // Wrap around
    ...
    const normalizedProgress = Math.abs(
      (currentPriceAnimationIndex / FLOW_ANI_INDEX_MAX) * 2 - 1,
    ); // 0 -> 1 -> 0
    const fadeStartOpacity = 10;
    const fadeMaxOpacity = 255 - fadeStartOpacity;
    const STATUS_IMG_ALPHA = Math.floor(
    ...
    const slashPosition =
      await DisplayComponents.renderPvPredictionTotal(device); // to get slash position
    ...
    if (!initialized) {
      ...
      return {
        payload: {
          ...
          device: DeviceManager.targetDeviceHost || 'Unknown',
