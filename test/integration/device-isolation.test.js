...
test('should maintain separate brightness for each device', async () => {
...
test('should maintain separate display power for each device', async () => {
  const logger = createMockLogger();
  const stateStore = createMockStateStore();
...
test('should not leak scene state between devices', async () => {
...
test('should continue operating device B when device A fails', async () => {
  const logger = createMockLogger();
  const stateStore = createMockStateStore();
...
test('should list working devices even if one device is broken', async () => {
  const logger = createMockLogger();
  const stateStore = createMockStateStore();
...
test('should not propagate errors from device A to device B', async () => {
  const logger = createMockLogger();
  const stateStore = createMockStateStore();
...
test('should handle concurrent brightness changes to different devices', async () => {
  const logger = createMockLogger();
  const stateStore = createMockStateStore();
...
test('should handle concurrent display power toggles', async () => {
  const logger = createMockLogger();
  const stateStore = createMockStateStore();
...
test('should handle mixed operations on different devices', async () => {
  const logger = createMockLogger();
  const stateStore = createMockStateStore();
...
test('should allow different drivers per device', async () => {
  const _logger = createMockLogger();
...
test('should not affect device B when switching driver on device A', async () => {
  const _logger = createMockLogger();
  const _stateStore = createMockStateStore();
...
test('should not corrupt device B state when removing device A', () => {
  const stateStore = createMockStateStore();
...
test('should maintain device B metrics after device A experiences errors', async () => {
  const logger = createMockLogger();
  const stateStore = createMockStateStore();
...
test('should handle 10 devices with independent states', async () => {
  const logger = createMockLogger();
  const stateStore = createMockStateStore();
...
test('should handle rapid state changes on multiple devices', async () => {
  const logger = createMockLogger();
  const stateStore = createMockStateStore();
