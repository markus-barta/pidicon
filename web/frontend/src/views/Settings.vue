    ...
    <v-row justify="center">
      <v-col cols="12" lg="11">
        ...
        >
          ...
          <v-window-item value="tests" v-if="showTestsTab">
            <v-card class="tab-card">
              <v-card-text class="pa-6">
                <div class="d-flex align-center justify-space-between mb-4">
                  <div>
                    <h2 class="text-h6 mb-1">Diagnostics Tests</h2>
                    <p class="text-body-2 text-medium-emphasis mb-0">
                      Run UI-facing diagnostics to verify hardware signals and daemon state.
                    </p>
                  </div>
...
export default {
  ...
  setup(props) {
    ...
    watch(
      () => props.showTestsTab,
      (enabled) => {
        if (enabled && diagnostics.value.tests.length === 0 && !diagnostics.value.loading) {
          loadDiagnostics();
        }
      },
      { immediate: true },
    ...
    return {
      ...
      showTestsTab: props.showTestsTab,
      diagnostics,
      diagnosticsHeaders,
      loadDiagnostics,
      runAllDiagnostics,
