/**
 * @fileoverview Professional Gradient Renderer - Advanced line drawing with gradient support
 * @description High-performance gradient line rendering with sophisticated color interpolation,
 * alpha blending, and professional-grade error handling. Extracted and optimized from legacy Node-RED implementation.
 * @version 1.0.0
 ...
 */
function validateGradientInputs(device, startPoint, endPoint, startColor, endColor) {
    ...
    if (!Array.isArray(startColor) || startColor.length < 3 ||
        !Array.isArray(endColor) || endColor.length < 3) {
 ...
 */
async function drawVerticalGradientLine(device, startPoint, endPoint, startColor, endColor, targetAlpha, alphaTarget = 'start') {
    ...
    const [physicalTopY, physicalBottomY] = startY <= endY ?
        [startY, endY] : [endY, startY];
    const [colorAtTop, colorAtBottom] = startY <= endY ?
    ...
    const useSpecialAlpha = typeof targetAlpha === 'number' &&
                           targetAlpha >= 0 && targetAlpha <= 255;

    // Pre-calculate color components for performance
    const [startR, startG, startB, startA] = colorAtTop;
    const [endR, endG, endB, endA] = colorAtBottom;
    ...
    for (let i = 0; i < lineLength; i++) {
        ...
        try {
            ...
            if (useSpecialAlpha && currentY === targetPixelY) {
                ...
                pixelColor = interpolateColor(colorAtTop, colorAtBottom, interpolationFactor);
            }

            await device.drawPixelRgba([x, currentY], pixelColor);

        } catch (error) {
            console.warn(`Gradient pixel render error at [${x}, ${currentY}]: ${error.message}`);
        }
    }
}
 ...
 */
async function drawHorizontalGradientLine(device, startPoint, endPoint, startColor, endColor, targetAlpha, alphaTarget = 'start') {
    validateGradientInputs(device, startPoint, endPoint, startColor, endColor);

    const [, y] = startPoint;
    const [startX, endX] = startPoint[0] <= endPoint[0] ?
        [startPoint[0], endPoint[0]] : [endPoint[0], startPoint[0]];
    const [colorAtLeft, colorAtRight] = startPoint[0] <= endPoint[0] ?
        [startColor, endColor] : [endColor, startColor];
    ...
    for (let i = 0; i < lineLength; i++) {
        ...
        try {
            ...
            if (useSpecialAlpha && currentX === targetPixelX) {
                ...
                // Calculate gradient color
                pixelColor = interpolateColor(colorAtLeft, colorAtRight, interpolationFactor);
            }

            await device.drawPixelRgba([currentX, y], pixelColor);

        } catch (error) {
            console.warn(`Gradient pixel render error at [${currentX}, ${y}]: ${error.message}`);
        }
    }
}
...
function createGradientRenderer(device) {
    return {
        ...
        async drawVerticalPreset(startPoint, endPoint, gradientType, targetAlpha) {
            ...
            return drawVerticalGradientLine(device, startPoint, endPoint,
                gradient.start, gradient.end, targetAlpha);
        },
         ...
         * @param {number} [targetAlpha] - Target alpha override
         */
        async drawHorizontalPreset(startPoint, endPoint, gradientType, targetAlpha) {
            const gradient = this.colors[gradientType];
            if (!gradient) {
                throw new Error(`Unknown gradient type: ${gradientType}`);
            }

            return drawHorizontalGradientLine(device, startPoint, endPoint,
                gradient.start, gradient.end, targetAlpha);
        },
