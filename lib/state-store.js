...
class StateStore {
   ...
   */
  constructor({ logger, persistPath = null } = {}) {
    ...
    this.persistPath = persistPath || '/data/runtime-state.json';
    this.persistEnabled = true;
    this.persistDirty = false;
    this.persistTimer = null;
    this.PERSIST_DEBOUNCE_MS = 10000; // Save every 10 seconds if dirty
  ...
  async restore() {
    ...
    try {
      ...
      if (!persistData.devices) {
        this.logger.warn('Invalid persisted state format, skipping restore');
        return;
      }
