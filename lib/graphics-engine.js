...
// Track devices that have already shown the capabilities warning (log once per device)
const _warningShownForDevices = new Set();

/**
 * Graphics Engine - Enhanced rendering system for pixel display devices
 * Provides text effects, fade transitions, and hardware-aware animations
 * Supports multiple device types with different dimensions
 */
class GraphicsEngine {
  constructor(device, capabilities = null) {
    if (!device) {
    ...
    // Device capabilities (dimensions, color depth, etc.)
    // If not provided, try to get from device, otherwise default to Pixoo64 dimensions
    if (capabilities) {
      this.capabilities = capabilities;
    } else if (device.capabilities) {
      this.capabilities = device.capabilities;
    } else {
      // Fallback to Pixoo64 dimensions for backward compatibility
      // Only log once per device to avoid spam (happens every frame)
      if (!_warningShownForDevices.has(this.host)) {
        logger.debug(
          `[GraphicsEngine] No capabilities provided for ${this.host}, using Pixoo64 defaults (64x64)`,
        );
        _warningShownForDevices.add(this.host);
      }
      this.capabilities = {
        width: 64,
        height: 64,
    ...
    this.width = this.capabilities.width;
    this.height = this.capabilities.height;
   ...
   * @param {Object} options.backdrop - Backdrop configuration
   * @param {number} options.backdrop.offset - Backdrop offset in pixels (default: 1)
   * @param {number[]} options.backdrop.color - Backdrop color [r, g, b, a] (default: [0,0,0,128])
  ...
  async drawTextEnhanced(text, position, color, options = {}) {
    const { alignment = 'left', effects = {}, backdrop = null } = options;
    const [x, y] = position;

    // Draw backdrop first if specified
    if (backdrop) {
      const { offset = 1, color: backdropColor = [0, 0, 0, 128] } = backdrop;
      ...
      await this.device.fillRect(
        [x - offset, y - offset],
        [textWidth + offset * 2, textHeight + offset * 2],
        backdropColor,
      );
    }
  ...
  async _drawTextGradient(text, position, baseColor, alignment /* effects */) {
    ...
    // For gradient text, we don't need backdrops since the layered effect
    // creates natural contrast. The gradient layers provide their own visibility.
  ...
  async drawImageBlended(
    ...
    if (blendMode === 'multiply') {
      ...
      // For images with black backgrounds (like moon PNGs), treat black as transparent

      try {
        // The key insight: multiply blend means black pixels (0) become transparent,
        // while brighter pixels are multiplied with the background.
        // Since we can't read background pixels, we use alpha to simulate this:
        // Very low alpha makes dark areas nearly invisible, bright areas more visible

        // Use a very low alpha to make dark/black pixels nearly transparent
        const multiplyAlpha = Math.round(alpha * 0.15); // 15% alpha for multiply effect
  ...
  async drawGradientBackground(startColor, endColor, direction = 'vertical') {
    ...
    if (direction === 'vertical') {
      // Vertical gradient (device-agnostic)
      for (let y = 0; y < this.height; y++) {
        const factor = y / (this.height - 1); // 0 to 1
        ...
        await this.device.fillRect([0, y], [this.width, 1], [r, g, b, a]);
      }
    } else {
      // Horizontal gradient (device-agnostic)
      for (let x = 0; x < this.width; x++) {
        const factor = x / (this.width - 1); // 0 to 1
        const r = Math.round(startR + (endR - startR) * factor);
        const g = Math.round(startG + (endG - startG) * factor);
        const b = Math.round(startB + (endB - startB) * factor);
        const a = Math.round(startA + (endA - startA) * factor);
        ...
        await this.device.fillRect([x, 0], [1, this.height], [r, g, b, a]);
      }
    }
  }
