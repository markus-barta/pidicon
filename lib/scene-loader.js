...
const LEGACY_LIB_REGEX = /^(\.\.\/)+lib\//;

Module._load = function patchedModuleLoad(request, parent, isMain) {
  if (typeof request === 'string' && LEGACY_LIB_REGEX.test(request)) {
    try {
      const stripped = request.replace(LEGACY_LIB_REGEX, 'lib/');
      const resolved = path.join(__dirname, '..', stripped);
      if (fs.existsSync(resolved) || fs.existsSync(`${resolved}.js`)) {
        return originalModuleLoad(resolved, parent, isMain);
      }
    } catch (error) {
      logger.debug('Legacy lib remap failed', {
        request,
        error: error.message,
      });
    }
