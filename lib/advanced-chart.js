 ...
 * @description Provides a class for rendering advanced charts with multiple
 * series, dynamic scaling, and customizable visual elements. It is designed
...
const CHART_DEFAULTS = Object.freeze({
  ...
  // Performance - calculated to fit within screen bounds
  // Screen: 64px, Margins: 2px left + 2px right = 60px available
  // Each bar: 2px (1px bar + 1px spacing), so max 30 bars
  MAX_DATA_POINTS: Math.floor((64 - 4) / 2),
...
class ChartDataProcessor {
  ...
  calculateStatistics(processedData) {
    ...
    return {
      ...
      avg:
        nonZeroPrices.reduce((sum, price) => sum + price, 0) /
        nonZeroPrices.length,
...
class AdvancedChartRenderer {
  ...
  async render(data, options = {}) {
    ...
    try {
      ...
      logger.error(`Chart render error: ${error.message}`);
  ...
  async renderAxes() {
    ...
    for (let x = LEFT; x < WIDTH - this.config.MARGINS.RIGHT; x++) {
      await this.device.drawPixelRgba(
        [x, HEIGHT - BOTTOM],
        this.config.AXIS_COLOR,
      );
    }
  }
  ...
  async renderGrid() {
    ...
    for (let y = TOP; y < HEIGHT - BOTTOM; y += gridSpacing) {
      for (let x = LEFT; x < WIDTH - RIGHT; x += 2) {
        // Dotted line
  ...
  async renderData(processedData, options = {}) {
    ...
    const zeroY =
      this.config.DIMENSIONS.HEIGHT - this.config.MARGINS.BOTTOM - 20;

    for (
      let i = 0;
      i < Math.min(data.length, this.config.MAX_DATA_POINTS);
      i++
    ) {
      const item = data[i];
  ...
  async renderLabels(metadata) {
    ...
    await this.device.drawTextRgbaAligned(
      scaleText,
      [2, 5],
      [128, 128, 128, 255],
      'left',
