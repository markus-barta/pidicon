    ...
    try {
      ...
      if (json.settings) {
        this.settings = {
          ...
          defaults: {
            driver:
              json.settings.defaults?.driver || this.settings.defaults.driver,
            brightness:
              json.settings.defaults?.brightness ??
              this.settings.defaults.brightness,
            watchdog: {
              timeoutMinutes:
                json.settings.defaults?.watchdog?.timeoutMinutes ??
                this.settings.defaults.watchdog.timeoutMinutes,
              action:
                json.settings.defaults?.watchdog?.action ||
                this.settings.defaults.watchdog.action,
              healthCheckIntervalSeconds:
                json.settings.defaults?.watchdog?.healthCheckIntervalSeconds ??
                this.settings.defaults.watchdog.healthCheckIntervalSeconds,
              checkWhenOff:
                json.settings.defaults?.watchdog?.checkWhenOff ??
                this.settings.defaults.watchdog.checkWhenOff,
            },
          },
        };
      }
