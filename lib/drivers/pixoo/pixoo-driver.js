...
class PixooDriver extends DeviceDriver {
  ...
  async setDisplayPower(on) {
    if (this.driverType === 'mock') {
      if (typeof this.device.setDisplayPower === 'function') {
        await this.device.setDisplayPower(on);
      }
      return true;
    }
    ...
    try {
      ...
      const logger = require('../../logger');
      logger.error(
        `Failed to set display power for ${this.host}: ${error.message}`
      );
      return false;
    }
  }
  ...
  /**
   * Health check - lightweight HTTP ping to verify device is alive
   * @returns {Promise<Object>} {success: bool, latencyMs: number}
   */
  async healthCheck() {
    if (this.driverType === 'mock') {
    ...
    try {
      ...
      // Phase 4 (Epic 0): Removed - this.stats.lastSeenTs update
