...
class AwtrixDriver extends DeviceDriver {
  ...
  async reboot() {
    if (this.driverType === 'mock') {
      this.logger.warn(`[AWTRIX Mock] Skipping reboot for ${this.host}`);
      return true;
    }

    try {
      const result = await this._httpRequest('POST', '/reboot', {});
      if (!result) {
        throw new Error('Device returned empty response');
      }
      this.logger.ok(`[AWTRIX] Reboot command acknowledged by ${this.host}`);
      return true;
    } catch (error) {
      this.logger.error(`[AWTRIX] Failed to reboot ${this.host}`, {
        error: error.message,
      });
      throw error;
    }
  ...
  getMetrics() {
    return {
      pushes: this.stats.messagesSent, // Map to standard 'pushes' field
      skipped: 0, // Awtrix doesn't skip frames
      errors: this.stats.errors,
      lastFrametime: 0, // Awtrix doesn't track frame time
  ...
  async _httpRequest(method, endpoint, payload = null) {
    ...
    try {
      ...
      // Log at DEBUG level - watchdog service handles connectivity issues with collated logging
      this.logger.debug(
        `[AWTRIX] HTTP ${method} ${endpoint} failed: ${error.message}`
      );
      return false;
    }
  }
  ...
  /**
   * Health check - HTTP GET to /stats to verify device is alive
   * @returns {Promise<Object>} {success: bool, latencyMs: number, stats?: Object}
   */
  async healthCheck() {
    if (this.driverType === 'mock') {
      // Mock: instant success with fake latency
    ...
    try {
      ...
      if (stats) {
        // Phase 4 (Epic 0): Removed - this.stats updates
        // This was causing crashes (this.stats undefined)
        // Watchdog service now manages health state via deviceHealth Map
  ...
  // ============================================================================
  // PIXOO-STYLE DRAWING PRIMITIVES (STUBS FOR COMPATIBILITY)
  // ============================================================================
  // These methods exist to prevent crashes when Pixoo scenes are accidentally
  // run on Awtrix devices. They log warnings and return gracefully.

  async drawRectangleRgba() {
    this.logger.warn(
      '[AWTRIX] drawRectangleRgba not supported - use device.drawCustom() instead'
    );
    return false;
  }

  async fillRectangleRgba() {
    this.logger.warn(
      '[AWTRIX] fillRectangleRgba not supported - use device.drawCustom() instead'
    );
    return false;
  }
  ...
  async drawPixelRgba() {
    this.logger.warn(
      '[AWTRIX] drawPixelRgba not supported - use device.drawCustom() instead'
    );
    return false;
  }
  ...
  async drawLineRgba() {
    this.logger.warn(
      '[AWTRIX] drawLineRgba not supported - use device.drawCustom() instead'
    );
    return false;
  }

  async drawTextRgbaAligned() {
    this.logger.warn(
      '[AWTRIX] drawTextRgbaAligned not supported - use device.drawCustom() instead'
    );
    return false;
  }
  ...
  async drawCustomFloatText() {
    this.logger.warn(
      '[AWTRIX] drawCustomFloatText not supported - use device.drawCustom() instead'
    );
    return false;
  }
  ...
  async drawImage() {
    this.logger.warn(
      '[AWTRIX] drawImage not supported - use device.drawCustom() instead'
    );
    return false;
  }
  ...
  async drawImageFile() {
    this.logger.warn(
      '[AWTRIX] drawImageFile not supported - use device.drawCustom() instead'
    );
    return false;
  }

  async drawImageWithAlpha() {
    this.logger.warn(
      '[AWTRIX] drawImageWithAlpha not supported - use device.drawCustom() instead'
    );
    return false;
  }
}
