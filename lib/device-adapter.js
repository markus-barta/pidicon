...
const _DRIVER_REGISTRY = {
  ...
  // AWTRIX driver will be added here when implemented
...
const DRIVER_DEFAULT = (
  process.env.PIDICO_DEFAULT_DRIVER ||
  process.env.PIXOO_DEFAULT_DRIVER ||
  'mock'
).toLowerCase();
...
if (useOverride) {
  ...
  TARGETS_RAW =
    process.env.PIDICO_DEVICE_TARGETS || process.env.PIXOO_DEVICE_TARGETS || '';
  logger.info('🌍 Using PIDICO_DEVICE_TARGETS from environment');
}
...
logger.debug(
  `   PIDICO_DEVICE_TARGETS env: "${process.env.PIDICO_DEVICE_TARGETS || process.env.PIXOO_DEVICE_TARGETS || 'not set'}"`,
);
...
if (useOverride) {
  ...
  process.env.PIDICO_DEVICE_TARGETS ||
