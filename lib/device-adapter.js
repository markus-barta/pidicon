...
const DEFAULT_BATCH_SIZE = 16; // safe small default
...
class DeviceProxy {
  ...
  async push(sceneName = "unknown", publishOk, mode = "full") {
    const start = Date.now();
  
    // --- Incremental mode ---
    if (mode === "incremental" && typeof this.impl.pushIncremental === "function") {
      try {
        const batchSize = sceneBatchSize || ctx?.defaultBatchSize || 16;
        const diffPixels = await this.impl.pushIncremental(batchSize);
        this.metrics.pushes++;
    ...
    try {
      ...
      if (publishOk)
        publishOk(this.host, sceneName, frametime, diffPixels, this.metrics);
      return diffPixels; // 👈 return number
    } catch (err) {
      this.metrics.errors++;
      throw err;
    }
