...
class DeviceProxy {
  constructor(host, size = 64, driver = "mock") {
    ...
    this.impl = null;
    this.lastFrame = null; // store last pushed frame
    this.metrics = { pushes: 0, skipped: 0, errors: 0 };
  ...
  async push(sceneName = "unknown", publishOk) {
    ...
    // If driver has no buf (e.g. MockDevice), just push
    if (!this.impl.buf) {
      try {
        await this.impl.push();
        this.metrics.pushes++;
        const frametime = Date.now() - start;
        if (publishOk) publishOk(this.host, sceneName, frametime, 0, this.metrics);
      } catch (err) {
        this.metrics.errors++;
        throw err;
      }
    ...
    const curr = this.impl.buf;
    let diffPixels = 0;

    if (this.lastFrame) {
      for (let i = 0; i < curr.length; i += 3) {
        if (
          curr[i] !== this.lastFrame[i] ||
          curr[i + 1] !== this.lastFrame[i + 1] ||
          curr[i + 2] !== this.lastFrame[i + 2]
        ) {
          diffPixels++;
        }
      }
    } else {
      // First frame → treat all pixels as changed
      diffPixels = (curr.length / 3) | 0;
    }
    ...
    if (diffPixels === 0) {
      ...
      const frametime = Date.now() - start;
      if (publishOk) publishOk(this.host, sceneName, frametime, 0, this.metrics);
      console.log(`⏩ No framebuffer change for ${this.host}, skipping push`);
      return;
    }

    // Clone current buffer for next comparison
    this.lastFrame = Buffer.from(curr);
