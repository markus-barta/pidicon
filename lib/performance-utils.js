/**
 * @fileoverview Performance Testing Utilities - Shared functions for performance test scenes
 * @description Common utilities for performance testing including color gradients, chart configurations,
 * and helper functions used across multiple performance test scenes.
 * @version 1.0.0
...
function validateSceneContext(ctx, sceneName) {
    ...
    if (missing.length > 0) {
        console.error(`❌ [${sceneName.toUpperCase()}] Missing required context properties: ${missing.join(', ')}`);
        return false;
    }
