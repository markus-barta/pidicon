...
const CHART_CONFIG = Object.freeze({
  // Layout
  START_Y: 50, // Legacy reference (bottom axis Y)
  RANGE_HEIGHT: 20,
  CHART_START_X: 1,
  CHART_START_Y: 16, // Top of chart area (keeps header text clear)
  CHART_BOTTOM_Y: 50, // Bottom axis Y position
  CHART_HEIGHT: 34, // Height from CHART_START_Y to CHART_BOTTOM_Y
  CHART_END_X: 62, // Right edge of chart drawing area
  CHART_WIDTH: 62, // Usable width (1..62)
  ...
  MAX_VALUE: 500, // Alias for MAX_FRAMETIME used by some scenes
});
...
function getPerformanceColor(frametime) {
  ...
  if (ratio <= 0.2) {
    ...
    color = [0, Math.round(200 * blend), Math.round(255 * blend), 255];
  } else if (ratio <= 0.4) {
    ...
    color = [255, Math.round(210 * (1 - blend)), 0, 255];
  } else {
    const blend = (ratio - 0.8) / 0.2;
    color = [255, Math.round(90 * (1 - blend)), 0, 255];
  }
...
// Named exports for ESM consumers (e.g., Vite frontend)
exports.CHART_CONFIG = CHART_CONFIG;
exports.COLOR_CACHE = COLOR_CACHE;
exports.getPerformanceColor = getPerformanceColor;
exports.getSimplePerformanceColor = getSimplePerformanceColor;
exports.isValidColor = isValidColor;
exports.validateSceneContext = validateSceneContext;
exports.formatPerformanceMetrics = formatPerformanceMetrics;
