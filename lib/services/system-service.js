...
class SystemService {
   ...
   * @param {Object} [dependencies.mqttConfigService] - MQTT configuration service
   */
  constructor({ logger, versionInfo, deploymentTracker, mqttConfigService }) {
    if (!logger) {
  ...
  async getStatus() {
    try {
      ...
      try {
        const cfg = await this.mqttConfigService?.loadConfig?.();
        if (cfg?.brokerUrl) {
          const { URL } = require('url');
          const url = new URL(cfg.brokerUrl);
          mqttBroker = url.hostname;
          mqttStatus.brokerUrl = cfg.brokerUrl;
        }
      } catch (err) {
        this.logger.warn('Failed to resolve MQTT broker from config', {
          error: err.message,
        });
      }

      if (this.mqttConfigService?.mqttService?.getStatus) {
        try {
          mqttStatus = {
            ...mqttStatus,
            ...this.mqttConfigService.mqttService.getStatus(),
          };
        } catch (err) {
          this.logger.warn('Failed to obtain MQTT runtime status', {
            error: err.message,
          });
        }
      }
      ...
      const daemonMetrics = this.stateStore?.getDaemonMetrics?.() || {
        startTime: null,
        lastHeartbeat: null,
      };
      const now = Date.now();
      const startTimeIso = daemonMetrics.startTime
        ? new Date(daemonMetrics.startTime).toISOString()
        : null;
      const lastHeartbeatIso = daemonMetrics.lastHeartbeat
        ? new Date(daemonMetrics.lastHeartbeat).toISOString()
        : null;
      const uptimeFromMetrics =
        daemonMetrics.startTime && daemonMetrics.startTime <= now
          ? Math.max(0, Math.floor((now - daemonMetrics.startTime) / 1000))
          : uptimeSeconds;
      const heartbeatAge =
        daemonMetrics.lastHeartbeat && daemonMetrics.lastHeartbeat <= now
          ? now - daemonMetrics.lastHeartbeat
          : null;
      const heartbeatStale = heartbeatAge !== null && heartbeatAge > 15000;
      ...
      const status = {
        ...
        uptimeTrackedSeconds: uptimeFromMetrics,
        uptimeTrackedFormatted: this._formatUptime(uptimeFromMetrics),
        daemonStartTime: startTimeIso,
        daemonLastHeartbeat: lastHeartbeatIso,
        daemonHeartbeatStale: heartbeatStale,
        memory: {
  ...
  async getLogs(lines = 50) {
    try {
      ...
      return [
        ...
        'For now, check Docker logs: docker logs pidicon',
      ];
    } catch (error) {
