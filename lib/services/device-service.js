...
class DeviceService {
   ...
   * @param {Object} dependencies.container - DI container for lazy service resolution (optional, Phase 3)
   */
  constructor({
    logger,
    deviceAdapter,
    ...
    this.container = container; // Optional - for lazy resolution (Phase 3)
  ...
  async getDeviceInfo(deviceIp) {
    try {
      ...
      this._logSceneStateDebug(deviceIp, sceneInternalState);

      const hardwareState = this._composeHardwareState(deviceIp, device);
      const deviceHardwareInfo = await this._getDeviceHardwareInfo(
        deviceIp,
        device
      );
      const deviceConfig = this.deviceConfigStore?.getDevice(deviceIp);

      // Phase 3 (Epic 0): Get watchdog health data
      let healthSummary = null;
      try {
        const watchdogService = this.container?.resolve?.('watchdogService');
        if (watchdogService) {
          healthSummary = watchdogService.getDeviceHealthSummary(deviceIp);
        }
      } catch (watchdogError) {
        // Watchdog might not be available yet during initialization
        this.logger.debug(
          `Watchdog health not available for ${deviceIp}:`,
      ...
      return {
        ...
        metrics: this._buildMetricsResponse(metrics),
        health: healthSummary, // New field (Phase 3)
        sceneState: this._buildSceneStateResponse(sceneInternalState),
        hardware: {
          brightness: hardwareState.brightness,
          displayOn: hardwareState.displayOn,
          loggingLevel: hardwareState.loggingLevel,
          ...deviceHardwareInfo, // Add device-specific info (e.g., batteryLevel for Awtrix)
        },
        config: this._buildDeviceConfigResponse(deviceConfig),
      };
    } catch (error) {
  ...
  _logSceneStateDebug(deviceIp, sceneInternalState = {}) {
    const { testCompleted, isRunning, framesRendered } = sceneInternalState;
    if (testCompleted || isRunning === false) {
  ...
  _composeHardwareState(deviceIp, device) {
    const hardwareDefaults = device.impl?.getHardwareState?.() || {};

    return {
      brightness:
        this.stateStore.getDeviceState(deviceIp, 'brightness') ??
        hardwareDefaults.brightness ??
        100,
      displayOn:
        this.stateStore.getDeviceState(deviceIp, 'displayOn') ??
        hardwareDefaults.displayOn ??
        true,
      loggingLevel: this.stateStore.getDeviceState(
        deviceIp,
        '__logging_level',
        null
      ),
  ...
  async _getDeviceHardwareInfo(deviceIp, device) {
    if (!(device.impl && typeof device.impl.getHardwareInfo === 'function')) {
      return {};
    }

    try {
      return (await device.impl.getHardwareInfo()) || {};
    } catch (error) {
      this.logger.debug(
  ...
  async setDisplayPower(deviceIp, on) {
    try {
      ...
      if (driver === 'mock') {
        ...
        // Get device instance and use driver's setDisplayPower method (device-type-aware)
        const device = this.deviceAdapter.getDevice(deviceIp);
        const result = await device.impl.setDisplayPower(on);

        if (!result) {
          throw new Error('Device did not accept display power command');
        }
      ...
      // Immediately flush critical state to ensure it's persisted before any daemon restart
  ...
  async resetDevice(deviceIp) {
    try {
      ...
      if (sceneState.currentScene && sceneState.currentScene !== 'none') {
        ...
        const sceneModule = this.sceneManager.getScene(sceneState.currentScene);
  ...
  async rebootDevice(deviceIp) {
    try {
      const driver = this.deviceAdapter.getDriverForDevice(deviceIp);
      const device = this.deviceAdapter.getDevice(deviceIp);
      const driverImpl = this.deviceAdapter.getDeviceDriverImpl
        ? this.deviceAdapter.getDeviceDriverImpl(deviceIp)
        : device?.impl;
      ...
      if (device.deviceType !== 'awtrix') {
        throw new Error('Reboot supported for AWTRIX devices only');
      }

      if (driver === 'mock') {
        this.logger.warn(
          `[MOCK MODE] ⛔ Reboot Command NOT SENT - Would reboot device ${deviceIp}`
        );
      } else if (driverImpl?.reboot) {
        await driverImpl.reboot();
      } else {
        throw new Error('Device implementation missing reboot capability');
      }

      this.logger.ok(`Device ${deviceIp} reboot triggered`);
      return {
        success: true,
        deviceIp,
        message: `Device reboot triggered${driver === 'mock' ? ' (mock mode)' : ''}`,
      };
    } catch (error) {
      this.logger.error(`Failed to reboot device ${deviceIp}:`, {
        error: error.message,
      });
      throw error;
    }
  ...
  /**
   * Rehydrates a device from persisted runtime state without overwriting the file
   * @param {string} deviceIp - Device IP address
   * @param {Object} persistedState - Persisted device state
   * @returns {Promise<Object>} Summary of rehydration result
   */
  async rehydrateFromState(deviceIp, persistedState = {}) {
    const result = {
      deviceIp,
      applied: [],
      skipped: [],
    ...
    const { displayOn, brightness, loggingLevel, activeScene, playState } =
      persistedState;

    try {
      if (displayOn !== undefined) {
        await this.setDisplayPower(deviceIp, Boolean(displayOn));
        result.applied.push('displayOn');
      } else {
        result.skipped.push('displayOn');
      }

      if (brightness !== undefined && brightness !== null) {
        await this.setDisplayBrightness(deviceIp, brightness);
        result.applied.push('brightness');
      } else {
        result.skipped.push('brightness');
      }

      if (loggingLevel) {
        await this.setDeviceLogging(deviceIp, loggingLevel);
        result.applied.push('loggingLevel');
      } else {
        result.skipped.push('loggingLevel');
      }

      if (activeScene) {
        const sceneModule = this.sceneManager.getScene(activeScene);
        if (sceneModule) {
          const context = this.deviceAdapter.getContext(
            deviceIp,
            activeScene,
          ...
          await this.sceneManager.switchScene(activeScene, context);
          result.applied.push('activeScene');

          if (playState === 'paused') {
            this.sceneManager.pauseScene(deviceIp);
            result.applied.push('playState:paused');
          } else if (playState === 'stopped') {
            await this.sceneManager.stopScene(deviceIp);
            result.applied.push('playState:stopped');
          } else if (playState === 'playing') {
            result.applied.push('playState:playing');
          }
        } else {
          this.logger.warn(
            `Persisted scene '${activeScene}' not found for ${deviceIp}, skipping`
          );
          result.skipped.push('activeScene');
        }
      } else {
        result.skipped.push('activeScene');
      }
    } catch (error) {
      this.logger.warn(`Failed to rehydrate ${deviceIp} from state`, {
        error: error.message,
        persistedState,
      });
      result.error = error;
  ...
  /**
   * Get device metrics (for watchdog health monitoring)
   * @param {string} deviceIp - Device IP address
   * @returns {Promise<Object>} Device metrics including lastSeenTs
   */
  async getMetrics(deviceIp) {
    try {
      const device = this.deviceAdapter.getDevice(deviceIp);
      if (!device || !device.getMetrics) {
        this.logger.debug(`Device ${deviceIp} not found or has no metrics`);
        return null;
      }

      return device.getMetrics();
    } catch (error) {
      this.logger.error(`Failed to get metrics for ${deviceIp}:`, {
        error: error.message,
      });
      return null;
    }
