...
class DeviceService {
  ...
  constructor({ logger, deviceAdapter, sceneManager, softReset }) {
    ...
    // Track hardware state for UI sync (UI-508)
    // Map<deviceIp, { brightness: number, displayOn: boolean }>
    this.deviceHardwareState = new Map();
  }
  ...
  async getDeviceInfo(deviceIp) {
    try {
      ...
      // Get hardware state for UI sync (UI-508)
      const hardwareState = this.deviceHardwareState.get(deviceIp) || {
        brightness: 100,
        displayOn: true,
      };
  ...
  async setDisplayPower(deviceIp, on) {
    try {
      ...
      // Track hardware state for UI sync (UI-508)
      const currentState = this.deviceHardwareState.get(deviceIp) || {
        brightness: 100,
        displayOn: true,
      };
      this.deviceHardwareState.set(deviceIp, {
        ...currentState,
        displayOn: on,
      });
  ...
  /**
   * Enable or disable debug logging for a specific device
   * @param {string} deviceIp - Device IP address
   * @param {boolean} enabled - Whether logging should be enabled
   * @returns {Promise<Object>} Result
   */
  async setDeviceLogging(deviceIp, enabled) {
    try {
      // Store logging preference in state store
      const key = `${deviceIp}:__logging_enabled`;
      this.stateStore.setState(key, enabled);

      this.logger.ok(
        `${enabled ? 'Enabled' : 'Disabled'} debug logging for ${deviceIp}`,
      );
      ...
      this.logger.error(`Failed to set logging for ${deviceIp}:`, {
        error: error.message,
        enabled,
      });
      throw error;
