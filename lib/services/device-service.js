...
class DeviceService {
   ...
   */
  constructor({ logger, deviceAdapter, sceneManager, stateStore, softReset }) {
    if (!logger) {
   ...
   * @param {string} deviceIp - Device IP address
   * @param {string} level - Log level: 'debug', 'warn', or 'none'
  ...
  async setDeviceLogging(deviceIp, level) {
    try {
      ...
      const key = `${deviceIp}:__logging_level`;
      this.stateStore.setState(key, level);

      const levelDescriptions = {
        debug: 'debug logging',
        warn: 'warning and error logging only',
        none: 'no logging',
      };
