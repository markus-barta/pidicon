...
class DeviceService {
  ...
  async getDeviceInfo(deviceIp) {
    try {
      ...
      const hardwareState = {
        brightness: this.stateStore.getDeviceState(deviceIp, 'brightness') ?? device.impl?.getHardwareState?.()?.brightness ?? 100,
        displayOn: this.stateStore.getDeviceState(deviceIp, 'displayOn') ?? device.impl?.getHardwareState?.()?.displayOn ?? true,
