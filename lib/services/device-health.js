...
class DeviceHealthEntry {
  ...
  updateLastSeen(timestamp = Date.now(), source = 'unknown') {
...
class DeviceHealthStore {
  ...
  updateLastSeen(host, timestamp = Date.now(), source = 'unknown') {
    return this.ensureEntry(host).updateLastSeen(timestamp, source);
  }
  ...
  getAllSnapshots() {
    ...
    for (const [host, entry] of this.entries.entries()) {
      snapshots[host] = entry.getSnapshot();
    }
    return snapshots;
  }
