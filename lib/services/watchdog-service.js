...
class WatchdogService {
  constructor(deviceConfigStore, deviceService, sceneService, stateStore) {
  ...
  async performHealthCheck(ip) {
    try {
      ...
      // Get device and perform health check
      const device = await this.deviceService.getDevice(ip);
      if (
        !device ||
        !device.impl ||
  ...
  async checkDevice(ip, timeoutMs) {
    try {
      ...
      if (!lastSeenTs) {
        // Device hasn't been seen yet, skip recovery check
        return;
      }
