...
class WatchdogService {
  ...
  async performHealthCheck(ip) {
    try {
      ...
      // Phase 4 (Epic 0): OLD system removed - only use new deviceHealth Map
      // Removed: device.health.updateLastSeen() - caused crashes when device.health undefined
      // New single source of truth: updateDeviceHealth() below
      ...
      // Phase 1-4 (Epic 0): Update device health state (single source of truth)
   ...
   * Epic 0, Story 0.2: Three-tier device state indicator
   *
   * Thresholds (from Story 0.2 spec):
   * - Responsive: 0-5 seconds
  ...
  _calculateDeviceStatus(healthState) {
    ...
    // Story 0.2 spec: Three-tier thresholds based on response time
    const RESPONSIVE_THRESHOLD_MS = 5 * 1000; // 5 seconds
    const DEGRADED_THRESHOLD_MS = 30 * 1000; // 30 seconds

    // Offline: Not seen for 30+ seconds
    if (timeSinceLastSeen > DEGRADED_THRESHOLD_MS) {
      return 'offline';
    }

    // Degraded: Seen within 30s but more than 5s ago
    if (timeSinceLastSeen > RESPONSIVE_THRESHOLD_MS) {
      return 'degraded';
    }

    // Online: Seen within last 5 seconds
    return 'online';
  }
