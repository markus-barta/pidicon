...
const { mergeSchemas } = require('../universal-scene-config');
...
class SceneService {
   ...
   * @param {Object} [dependencies.deviceConfigStore] - Device config store (optional, for scene defaults)
   */
  constructor({
    logger,
    sceneManager,
    ...
    this.deviceConfigStore = deviceConfigStore; // Optional: for scene defaults
  }
  ...
  async switchToScene(deviceIp, sceneName, options = {}) {
    ...
    try {
      ...
      // Merge device-specific scene defaults with payload
      // Priority: scene defaults < device overrides < MQTT payload
      let mergedPayload = { ...payload };
      if (this.deviceConfigStore) {
        try {
          const deviceConfig = this.deviceConfigStore.getDevice(deviceIp);
          if (deviceConfig) {
            const deviceDefaults = deviceConfig.getSceneDefaults(sceneName);
            if (deviceDefaults) {
              mergedPayload = { ...deviceDefaults, ...payload };
              this.logger.debug(
                `Applied device-specific defaults for scene ${sceneName}`,
                {
                  deviceDefaults,
                  payload,
          ...
          this.logger.warn(`Failed to load device defaults for ${deviceIp}:`, {
            error: error.message,
          });
          // Continue with original payload if device defaults couldn't be loaded
        }
      }

      // Get device context (pass publishOk for WebSocket broadcasts)
      const sceneModule = this.sceneManager.getScene(sceneName);
      ...
      // Add scene config (merged payload) to context for timing/scheduling
      context.sceneConfig = mergedPayload;
      ...
      // Track scene usage (after successful switch)
      if (this.deviceConfigStore) {
        try {
          const deviceConfig = this.deviceConfigStore.getDevice(deviceIp);
          if (deviceConfig) {
            deviceConfig.trackSceneUsage(sceneName);
            await this.deviceConfigStore.saveConfig();
            this.logger.debug(
              `Tracked usage for scene ${sceneName} on ${deviceIp}`
            );
          }
        } catch (error) {
          this.logger.warn(`Failed to track scene usage:`, {
            error: error.message,
          });
          // Non-fatal, continue
        }
    ...
    try {
      ...
      for (const sceneName of sceneNames) {
        ...
        scenes.push({
          ...
          tags: Array.isArray(sceneModule.tags) ? sceneModule.tags : [],
          isDevScene: sceneModule.isDevScene === true,
          deviceTypes: Array.isArray(sceneModule.deviceTypes)
            ? sceneModule.deviceTypes
            : ['pixoo64', 'pixoo'],
  ...
  /**
   * Get list of all available scenes with their config schemas
   * Includes universal timing parameters merged with scene-specific schemas
   * @returns {Promise<Array<Object>>} List of scene objects with configSchema included
   */
  async listScenesWithSchema() {
    try {
      // Use the new listScenesWithMetadata method that includes all metadata
      const scenes = this.sceneManager.listScenesWithMetadata();

      // Merge universal schema with scene-specific schema and normalize paths
      const scenesWithSchema = scenes.map((scene) => {
        const sceneModule = this.sceneManager.scenes.get(scene.name);
        const normalizedPath = normalizeScenePath(
          sceneModule.filePath,
          scene.name
        );

        // Merge universal parameters with scene-specific config schema
        const mergedSchema = mergeSchemas(scene.configSchema);

        return {
          ...scene,
          configSchema: mergedSchema,
          filePath: normalizedPath,
          isDevScene: scene.sceneType === 'dev', // Map sceneType to isDevScene for backward compatibility
        };
      });

      this.logger.debug(`Listed ${scenesWithSchema.length} scenes with schema`);
      return scenesWithSchema;
    } catch (error) {
      this.logger.error('Failed to list scenes with schema:', {
        error: error.message,
      });
      throw error;
    }
  ...
  /**
   * Get config schema for a specific scene
   * @param {string} sceneName - Scene name
   * @returns {Object|null} Config schema or null if scene not found or has no schema
   */
  getSceneConfigSchema(sceneName) {
    try {
      if (!this.sceneManager.hasScene(sceneName)) {
        this.logger.warn(`Scene not found: ${sceneName}`);
        return null;
      }

      const sceneModule = this.sceneManager.scenes.get(sceneName);
      return sceneModule.configSchema || null;
    } catch (error) {
      this.logger.error(`Failed to get config schema for ${sceneName}:`, {
        error: error.message,
      });
      return null;
    }
  ...
  async resumeScene(deviceIp) {
    try {
      ...
      const resumeSceneModule = this.sceneManager.getScene(sceneName);
      const resumeContext = this.deviceAdapter.getContext(
        deviceIp,
        sceneName,
        {},
        null,
      ...
      const success = await this.sceneManager.resumeScene(
        deviceIp,
        resumeContext
      );
