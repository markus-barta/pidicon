...
const CATEGORY_PATTERNS = {
  [`test${path.sep}lib${path.sep}`]: 'unit-tests',
  [`test${path.sep}integration${path.sep}`]: 'integration-tests',
  [`test${path.sep}contracts${path.sep}`]: 'contract-tests',
  [`ui-tests${path.sep}`]: 'ui-tests',
};

const DEFAULT_CATEGORY_COUNTS = Object.fromEntries(
  Object.keys(CATEGORY_PREFIXES).map((category) => [category, 0])
);
...
class TestResultsParser {
  ...
  async #loadRegistry() {
    try {
      ...
      this.registry = {
        ...
        tests: {},
        counters: { ...DEFAULT_CATEGORY_COUNTS },
      };
    }
  }
  ...
  #getTestId(filePath, testName, category) {
    ...
    if (!this.registry.tests[hash]) {
      const prefix =
        CATEGORY_PREFIXES[category] || CATEGORY_PREFIXES['unit-tests'];
      if (typeof this.registry.counters[category] !== 'number') {
        this.registry.counters[category] = 0;
      }
      this.registry.counters[category] += 1;
      this.registry.tests[hash] =
        `${prefix}-${this.registry.counters[category]}`;
    }
  ...
  #getCategoryFromPath(filePath) {
    ...
    if (normalizedPath.includes('/diagnostics/')) {
      return 'system';
    }
