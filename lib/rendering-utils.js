/**
 * @fileoverview Enhanced text rendering utilities with pixel-perfect backdrop support
 * @description Professional text rendering with configurable backdrop offsets for improved readability
 * @author Markus Barta (mba) with assistance from Cursor AI
 * @license MIT
 */

/**
 * @fileoverview Rendering Utilities - Professional drawing functions for Pixoo devices
 * @description High-quality rendering utilities with pixel-perfect text backdrops and smooth graphics
 * @author Markus Barta (mba) with assistance from Cursor AI
 * @license MIT
 ...
 * );
 */
async function drawTextWithPixelPerfectBackdrop(
  device,
  text,
  pos,
  textColor,
  ...
  // Calculate exact text dimensions using device rendering
  const textMetrics = calculateTextBounds(str, align, x, y);

  // Apply configurable offset
  const offsetX = Math.max(0, Math.floor(offset));
  const offsetY = Math.max(0, Math.floor(offset));
  ...
  // Draw backdrop rectangle with exact bounds
  if (bgColor && Array.isArray(bgColor) && bgColor.length === 4) {
  ...
  await device.drawTextRgbaAligned(text, [x, y], textColor, align);
 ...
 */
function calculateTextBounds(text, align, x, y) {
  const str = String(text);

  // Enhanced character width calculation for pixel-perfect bounds
  let estimatedWidth = 0;
  for (const char of str) {
    if (char === ' ' || char === ':') estimatedWidth += 3;
    else if (char === 'M' || char === 'W') estimatedWidth += 5;
    else if (char >= '0' && char <= '9') estimatedWidth += 4;
    else estimatedWidth += 4; // Default for letters and punctuation
  }

  // Ensure minimum width and clamp to device limits
  const width = Math.max(1, Math.min(64, estimatedWidth));
  const height = 7; // Standard text height (font is 5px + 2px padding)

  // Calculate exact bounds based on alignment
  let boundsX;
  switch (align) {
    case 'center':
      boundsX = Math.max(0, x - Math.floor(width / 2));
      break;
    case 'right':
  ...
  return {
    x: boundsX,
    y: y, // Y position is already the top of the text (same as input y)
    width: width,
    height: height,
  };
}
 ...
 * @deprecated Use drawTextWithPixelPerfectBackdrop for better control
 */
...
function createTextRenderer(device) {
  ...
  return {
     ...
     */
    renderWithBackdrop: (text, pos, color, align, bgColor, offset = 1) =>
     ...
     * @param {number} y - Y position
     */
    calculateBounds: (text, align, x, y) =>
      calculateTextBounds(text, align, x, y),
