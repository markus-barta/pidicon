 ...
 * @license MIT
 * @stability Production-grade with exact font metrics
 */
...
/**
 * Exact font metrics for pixel-perfect text rendering
 * Based on the Pixoo device's 5x7 monospace font with exact character bounds
 * Format: {width, height, baseline} where baseline is the bottom of the character
 */
const FONT_METRICS = Object.freeze({
  // Standard font dimensions
  BASELINE: 5, // Characters sit on this line (0-4 pixels)
  LINE_HEIGHT: 7, // Total height including spacing (5px char + 2px padding)

  // Character-specific width metrics (exact pixel widths)
  CHARS: {
    // Special characters
    ' ': { width: 2 }, // Space is narrow
    '!': { width: 1 },
    '"': { width: 3 },
    '#': { width: 5 },
    $: { width: 4 },
...
function measureText(text, alignment, x, y) {
  ...
  // Calculate exact total width using precise character metrics
  let totalWidth = 0;
  for (const char of str) {
    const charMetrics = FONT_METRICS.CHARS[char];
    totalWidth += charMetrics ? charMetrics.width : 4; // fallback to 4px for unknown chars
  }

  // Use exact font metrics for dimensions
  const width = Math.max(1, Math.min(64, totalWidth));
  const height = FONT_METRICS.LINE_HEIGHT; // Use exact line height from font metrics
