/**
 * @fileoverview Shared Rendering Utilities - Professional-grade rendering functions for all Pixoo scenes
 * @description Common rendering utilities with intelligent background clearing, optimized drawing algorithms,
 * and consistent API across all scenes. This module provides the foundation for clean, maintainable scene rendering.
 * @version 1.0.0
 ...
 * const { drawTextRgbaAlignedWithBg, BACKGROUND_COLORS } = require('../lib/rendering-utils');
 *
 ...
 */
async function drawTextRgbaAlignedWithBg(device, text, pos, color, align = "left", clearBg = false, bgColor = [0, 0, 0, 255]) {
    if (!device || typeof device.drawTextRgbaAligned !== 'function') {
        throw new Error('Invalid device interface - missing drawTextRgbaAligned method');
    }
    ...
    if (clearBg) {
        ...
        if (typeof device.drawRectangleRgba === 'function') {
            ...
            const validBgColor = Array.isArray(bgColor) && bgColor.length === 4
                ? bgColor
...
function createTextRenderer(device) {
    return {
        ...
        async renderStatus(text, pos, style = "primary") {
            const color = this.colors[style] || this.colors.primary;
            return drawTextRgbaAlignedWithBg(device, text, pos, color, "center", true);
        },
        ...
        async clearArea(x, y, width, height) {
            return clearRectangle(device, x, y, width, height, this.colors.background);
        }
    };
}
...
function createChartRenderer(device) {
    return {
        ...
        async renderAxes(startY, rangeHeight, startX = 1) {
            ...
            await clearRectangle(device, 0, startY - rangeHeight, 1, rangeHeight, this.colors.background);
            await clearRectangle(device, startX, startY, 64 - startX, 1, this.colors.background);
