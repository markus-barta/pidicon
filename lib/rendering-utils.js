 ...
 * @param {Object} device - Device instance with drawPixel method
 * @param {Array} center - [x, y] center position
 * @param {number} radius - Circle radius
 * @param {Array} color - [r, g, b, a] color
 */
async function drawFilledCircle(device, center, radius, color) {
  const [cx, cy] = center;
    ...
    for (let dy = -radius; dy <= radius; dy++) {
      if (dx * dx + dy * dy <= radius * radius) {
        ...
        if (x >= 0 && x < 64 && y >= 0 && y < 64) {
          await device.drawPixel([x, y], color);
        }
      }
    }
  }
 ...
 */
async function drawCircleOutline(device, center, radius, color, thickness = 1) {
  const [cx, cy] = center;
  const innerRadiusSq = Math.max(
    0,
...
/**
 * Draw a circle with gradient fill (radial gradient)
 * @param {Object} device - Device instance with drawPixel method
 * @param {Array} center - [x, y] center position
 * @param {number} radius - Circle radius
 * @param {Array} centerColor - [r, g, b, a] color at center
 * @param {Array} edgeColor - [r, g, b, a] color at edge
 */
async function drawGradientCircle(
  device,
  center,
  radius,
  centerColor,
  ...
  for (let dx = -radius; dx <= radius; dx++) {
    for (let dy = -radius; dy <= radius; dy++) {
      ...
      if (distSq <= radius * radius) {
        ...
        const factor = dist / radius; // 0 at center, 1 at edge
        const r = Math.round(
          centerColor[0] + (edgeColor[0] - centerColor[0]) * factor,
        );
        const g = Math.round(
          centerColor[1] + (edgeColor[1] - centerColor[1]) * factor,
        );
        const b = Math.round(
          centerColor[2] + (edgeColor[2] - centerColor[2]) * factor,
        );
        const a = Math.round(
          centerColor[3] + (edgeColor[3] - centerColor[3]) * factor,
        );

        const x = cx + dx;
        const y = cy + dy;
        if (x >= 0 && x < 64 && y >= 0 && y < 64) {
          await device.drawPixel([x, y], [r, g, b, a]);
        }
      }
    }
  }
 ...
 * @param {Object} device - Device instance with drawPixel method
 * @param {Array} center - [x, y] center position
 * @param {number} radius - Core circle radius
 * @param {number} glowRadius - Total radius including glow
 * @param {Array} color - [r, g, b, a] core color
 */
async function drawGlowCircle(device, center, radius, glowRadius, color) {
  const [cx, cy] = center;

  // Draw glow first (outer to inner for proper layering)
  for (let dx = -glowRadius; dx <= glowRadius; dx++) {
    for (let dy = -glowRadius; dy <= glowRadius; dy++) {
      const distSq = dx * dx + dy * dy;
      if (distSq > radius * radius && distSq <= glowRadius * glowRadius) {
        const dist = Math.sqrt(distSq);
        const glowFactor = 1 - (dist - radius) / (glowRadius - radius);
        const alpha = Math.round(color[3] * glowFactor * 0.5); // 50% max for glow

        const x = cx + dx;
        const y = cy + dy;
        if (x >= 0 && x < 64 && y >= 0 && y < 64) {
          await device.drawPixel([x, y], [color[0], color[1], color[2], alpha]);
        }
      }
    }
  }
  ...
  // Draw core circle
  await drawFilledCircle(device, center, radius, color);
}
