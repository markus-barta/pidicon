...
class DeviceContextFactory {
  constructor(mqttClient, stateStore = null) {
  ...
  createContext(deviceIp, sceneName, payload = {}, publishOk = null) {
    try {
      ...
      const context = {
        ...
        // Conditional logging based on device preference (cached for performance)
        // Logging levels: debug, info, warning, error, silent
        log: (() => {
          // Cache the logging level to avoid repeated state store lookups
          let cachedLogLevel = null;
          let lastCheck = 0;
          const CACHE_TTL = 5000; // Refresh cache every 5 seconds

          return (message, level = 'info', meta = {}) => {
            try {
              // Refresh cache if expired
              const now = Date.now();
              if (cachedLogLevel === null || now - lastCheck > CACHE_TTL) {
                const deviceLogLevel = this.stateStore
                  ? this.stateStore.getDeviceState(deviceIp, '__logging_level')
                  : null;
              ...
              const configLevel = levelHierarchy[cachedLogLevel] || 4; // Default to silent
              ...
              const shouldLog =
                messageLevel >= configLevel && cachedLogLevel !== 'silent';
              ...
              if (shouldLog) {
                ...
                logMethod(`[${sceneName}@${deviceIp}] ${message}`, meta);
              }
            } catch (error) {
