  ...
  createContext(deviceIp, sceneName, payload = {}, publishOk = null) {
    try {
      ...
      const context = {
        ...
        // Conditional logging based on device preference (no cache for immediate response)
        // Logging levels: debug, info, warning, error, silent
        log: (() => {
          return (message, level = 'info', meta = {}) => {
            try {
              // Get current logging level (no cache for immediate response to level changes)
