      ...
      const context = {
        ...
        log: (message, meta = {}) => {
          try {
            ...
            const loggingKey = `${deviceIp}:__logging_enabled`;
            const loggingEnabled = this.stateStore
              ? this.stateStore.getState(loggingKey)
              : null;

            // Default: enabled for real devices, disabled for mock devices
            const shouldLog =
              loggingEnabled !== null
                ? loggingEnabled
            ...
            if (shouldLog) {
              logger.info(`[${sceneName}@${deviceIp}] ${message}`, meta);
            }
          } catch (error) {
