  ...
  createContext(deviceIp, sceneName, payload = {}, publishOk = null) {
    try {
      ...
      const context = {
        ...
        // Conditional logging based on device preference
        log: (message, level = 'info', meta = {}) => {
          try {
            // Get device's logging level setting
            const loggingKey = `${deviceIp}:__logging_level`;
            const deviceLogLevel = this.stateStore
              ? this.stateStore.getState(loggingKey)
              : null;

            // Default: 'warn' for real devices, 'none' for mock devices
            const effectiveLogLevel = deviceLogLevel !== null
              ? deviceLogLevel
              : (device.currentDriver === 'real' ? 'warn' : 'none');
            ...
            const shouldLog = effectiveLogLevel === 'debug' ||
                             (effectiveLogLevel === 'warn' && (level === 'warn' || level === 'error')) ||
