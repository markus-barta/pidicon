...
const { UNIVERSAL_CONFIG_SCHEMA } = require('./universal-scene-config');
...
class SceneManager {
  ...
  constructor({ logger, stateStore } = {}) {
    ...
    this.devicePlayState = new Map(); // host -> 'playing'|'paused'|'stopped' (always used)
    this.sceneStates = new Map(); // sceneName -> Map (fallback)
    this.deviceSceneStartTime = new Map(); // host -> timestamp (for timeout tracking)
   ...
   * @private
   */
  _getSceneState(sceneName, deviceId = null) {
    if (this.stateStore && deviceId) {
      // Initialize and return the actual Map from StateStore
      this.stateStore.initSceneState(deviceId, sceneName);
      return this.stateStore.getScene(deviceId, sceneName);
    }

    if (this.stateStore) {
      // StateStore is in use but no deviceId provided
      // Return empty Map for backward compatibility
      return new Map();
    }

    // Legacy Map access (when StateStore is not injected)
    const key = sceneName;
  ...
  async switchScene(sceneName, context) {
    try {
      ...
      this._setDeviceState(host, 'playState', 'playing');

      // Record scene start time for timeout tracking
      this.deviceSceneStartTime.set(host, Date.now());
      ...
      if (wantsLoop) {
        ...
        const deviceLoop = async () => {
          ...
          const currentPlayState = this._getDeviceState(
            host,
            'playState',
            'stopped'
          );
          ...
          // Check scene timeout (if configured)
          const universalParams = context.sceneConfig || {};
          const timeoutMinutes = universalParams.sceneTimeout;
          if (timeoutMinutes) {
            const startTime = this.deviceSceneStartTime.get(host);
            if (startTime) {
              const elapsedMs = Date.now() - startTime;
              const elapsedMinutes = elapsedMs / (1000 * 60);
              if (elapsedMinutes >= timeoutMinutes) {
                this.logger.info(
                  `Scene timeout reached for ${sceneName} (${elapsedMinutes.toFixed(1)}m >= ${timeoutMinutes}m)`
                );
                this.deviceLoopTimers.delete(host);
                this.deviceSceneStartTime.delete(host);
                return;
              }
            }
          }
          ...
          // Apply universal timing parameters
          const renderInterval =
            universalParams.renderInterval ||
            UNIVERSAL_CONFIG_SCHEMA.renderInterval.default;
          const adaptiveTiming =
            universalParams.adaptiveTiming !== undefined
              ? universalParams.adaptiveTiming
              : UNIVERSAL_CONFIG_SCHEMA.adaptiveTiming.default;

          let delay = nextDelayMs;

          // If adaptive timing enabled, measure frame time and adjust
          if (adaptiveTiming) {
            const frameTime = Date.now() - frameStartTime;
            // Use the max of scene's requested delay or remaining time until next interval
            const remainingTime = Math.max(0, renderInterval - frameTime);
            delay = Math.max(nextDelayMs, remainingTime);
          } else {
            // Non-adaptive: use configured render interval
        ...
        this.logger.info(
          `▶️  Starting loop for ${sceneName} (gen=${newGen}, playState=${this._getDeviceState(host, 'playState', 'playing')})`
        );
        await deviceLoop();
      } else {
  ...
  pauseScene(host) {
    try {
      ...
      this._setDeviceState(host, 'playState', 'paused');
  ...
  async resumeScene(host, context) {
    try {
      ...
      const playState = this._getDeviceState(host, 'playState', 'stopped');
      ...
      // Reset scene start time on resume
      this.deviceSceneStartTime.set(host, Date.now());
      ...
      const deviceLoop = async () => {
        ...
        const curPlayState = this._getDeviceState(host, 'playState', 'stopped');
        ...
        if (timeoutMinutes) {
          ...
          if (startTime) {
            ...
            if (elapsedMinutes >= timeoutMinutes) {
              this.logger.info(
                `Scene timeout reached for ${activeScene} (${elapsedMinutes.toFixed(1)}m >= ${timeoutMinutes}m)`
              );
              this.deviceLoopTimers.delete(host);
              this.deviceSceneStartTime.delete(host);
              return;
  ...
  stopScene(host) {
    try {
      ...
      this._setDeviceState(host, 'playState', 'stopped');
  ...
  async renderActiveScene(context) {
    ...
    try {
      const scene = this.getScene(sceneName);
      // Pass deviceId to _getSceneState so it can create a proper proxy for StateStore
      const state = this._getSceneState(sceneName, host);
      ...
      // Check if error is due to incompatible device/scene
      const isIncompatibleError =
        error.message?.includes('is not a function') ||
        error.message?.includes('not supported');

      if (isIncompatibleError) {
        const host = context.device?.ip || 'unknown';
        this.logger.error(
          `🚫 Scene "${sceneName}" is incompatible with this device. Stopping scene.`
        );

        // Store error in device state for UI display
        this._setDeviceState(host, 'sceneError', {
          message: `Scene "${sceneName}" is incompatible with this device type`,
          detail: error.message,
          timestamp: Date.now(),
        });

        // Stop the scene to prevent further errors
        this._setDeviceState(host, 'playState', 'stopped');
      }
   ...
   * @returns {Array<Object>} Array of scene objects with metadata
   */
  listScenesWithMetadata() {
    const scenes = [];
    for (const [name, scene] of this.scenes.entries()) {
      scenes.push({
        ...
        tags: scene.tags || [],
        configSchema: scene.configSchema || null,
        // New metadata fields
        sceneType: scene.sceneType || 'user',
        author: scene.author || 'Unknown',
        version: scene.version || '1.0.0',
        thumbnail: scene.thumbnail || null,
        isHidden: scene.isHidden || false,
        sortOrder: scene.sortOrder !== undefined ? scene.sortOrder : 1000,
        wantsLoop: !!scene.wantsLoop,
      });
    }
    return scenes;
  }
  ...
  getDeviceSceneState(host) {
    return {
      ...
      playState: this._getDeviceState(host, 'playState', 'stopped'),
    };
  }
