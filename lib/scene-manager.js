...
class SceneManager {
  ...
  async updateSceneParameters(sceneName, context) {
    try {
      ...
      // Same scene, just update context and re-render
  ...
  async renderActiveScene(context) {
    if (!this.activeScene) {
      this.logger.warn('⚠️ No active scene to render');
      return false;
    }

    try {
      const scene = this.getScene(this.activeScene);
      const state = this.getSceneState(this.activeScene);

      const sceneContext = {
        ...context,
        state,
        sceneName: this.activeScene,
      };
      ...
      this.logger.error(
        `❌ Scene render failed for ${this.activeScene}:`,
        error.message,
      );
      return false;
    }
