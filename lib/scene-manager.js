...
class SceneManager {
  constructor() {
    this.scenes = new Map();
    this.activeScenes = new Map(); // deviceIp -> sceneName
  ...
  async switchScene(sceneName, context) {
    try {
      ...
      if (this.activeScene && this.activeScene !== sceneName) {
        // Build a proper context tied to the previous scene so its getState/setState
        // operate on the correct host::scene storage.
        try {
          const { getContext } = require('./device-adapter');
          const host = context?.env?.host;
          const cleanupCtx = host
            ? getContext(host, this.activeScene, {}, context?.publishOk)
            : { ...context, sceneName: this.activeScene };
          await this.cleanupScene(this.activeScene, cleanupCtx);
        } catch {
          // Fallback: try cleanup with provided context
      ...
      this.logger.ok(`Scene switched to: ${sceneName}`);
