...
class SceneManager {
  ...
  _getSceneState(sceneName, deviceId = null) {
    if (this.stateStore && deviceId) {
      // Return a Map-like proxy that delegates to StateStore
      // This allows scenes to use state.get(key) and state.set(key, value)
      const stateStore = this.stateStore;

      // Create a proxy Map that delegates to StateStore
      const proxyMap = {
        get: (key, defaultValue = undefined) => {
          return stateStore.getSceneState(
            deviceId,
            sceneName,
            key,
            defaultValue
        ...
        set: (key, value) => {
          stateStore.setSceneState(deviceId, sceneName, key, value);
          return proxyMap;
        },
        has: (key) => {
          return (
            stateStore.getSceneState(deviceId, sceneName, key, null) !== null
          );
        },
        delete: (key) => {
          stateStore.setSceneState(deviceId, sceneName, key, undefined);
          return true;
        },
        clear: () => {
          stateStore.clearSceneState(deviceId, sceneName);
        },
      };
