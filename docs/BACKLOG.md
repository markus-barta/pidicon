...
### SYS-415: Smart Release Checker Caching (P2)

- **Priority**: P2 (Nice to Have)
- **Effort**: 2-4 hours
- **Owner**: mba
- **Status**: Backlog (2025-10-31)

**Problem**: ReleaseChecker now fetches from GitHub Pages on every check (no caching). For production with many users or frequent checks, this could create unnecessary network requests.

**Goal**: Re-implement intelligent caching that balances freshness with efficiency:
...
**Notes**: Cache was removed in build #902 to fix stale deployment visibility. Only re-add if production traffic justifies it.

---

### OPS-412: Reliable Daemon Uptime Signal (P1)
